...
foreach($row in Get-Content input.txt) {
    ...
    if([int]$numbers[0] -lt [int]$numbers[2]) {
        ...
        Write-Debug "[Partial] slacker! Since $($numbers[0]) is the same as $($numbers[2]), it's implicit that one contains entirely the other"
        $slacking_elves++
        $partial_slacking_elves++
    }
}

Write-Output "There are $slacking_elves elves with nothing to do and $partial_slacking_elves elves with some overlap."